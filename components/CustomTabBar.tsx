...
export function CustomTabBar({ state, descriptors, navigation }: BottomTabBarProps) {
  ...
  const tabWidth = (width - 32) / 5; // 5 tabs, container is width - 32 (padding left/right 16)
  ...
  useEffect(() => {
    // Slide the active indicator capsule
    activeTabX.value = withSpring(state.index * tabWidth, {
      damping: 16,
      stiffness: 140,
    });
  }, [state.index]);
          ...
          return (
            ...
            >
              ...
              <Text
                ...
                className={`mt-1 ${isFocused ? "text-text" : "text-muted"}`}
              >
                {label}
              </Text>
            </Pressable>
...
function AnimatedIcon({ name, isFocused }: { name: IconName; isFocused: boolean }) {
  ...
  return (
    <Animated.View style={animatedStyle}>
      <Icon name={name} size={20} color={isFocused ? "#FFFFFF" : "#8E8E93"} />
    </Animated.View>
  );
}
