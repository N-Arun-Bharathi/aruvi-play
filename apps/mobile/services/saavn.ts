...
export async function getLyrics(
  ...
  // 1. Try JioSaavn native lyrics API
  try {
    ...
    if (data && data.lyrics && typeof data.lyrics === "string" && data.lyrics.trim().length > 0) {
      const cleaned = cleanSaavnLyrics(data.lyrics);
      const lyricsData: LyricsData = {
        lyrics: cleaned.lyrics,
        lines: cleaned.lines,
        snippet: data.snippet ? decodeHtml(data.snippet) : undefined,
        copyright: data.lyrics_copyright ? decodeHtml(data.lyrics_copyright) : "Lyrics powered by JioSaavn",
        isSynced: false,
        source: "jiosaavn",
      };
    ...
    console.warn("Mobile JioSaavn lyrics lookup failed, attempting fallback:", err);
  }
